...
if ($action eq "maj") {
    ...
    $action="";
	print "Mise a jour effectuee<br>";
}
	
sub update_prod(){	
    $vendu_retoursql=&get("select sum(ret_qte-ret_retour) from retoursql where ret_code ='$appro' and ret_cd_pr='$prod'","af")+0;
    $vendu_douane=&get("select es_qte from enso where es_no_do='$appro' and es_cd_pr='$prod'","af")+0;
    $vendu_retoursql*=100;
    $ecart=$vendu_douane-$vendu_retoursql;
    $datesimple=&get("select curdate()");
    &save("update produit set pr_stre=pr_stre+$ecart where pr_cd_pr='$prod'","af");
    if ($vendu_retoursql==0){
      &save("delete from enso where es_cd_pr='$prod' and es_no_do='$appro'","af");
      &save("delete from rotation where ro_code='$appro' and ro_rot='1' and ro_cd_pr='$prod' ","af");
    }
    else
    {
	  $date_orig=&get("select es_dt from enso where es_cd_pr='$prod' and es_no_do='$appro'");
      if ($date_orig ne ""){$datesimple=$date_orig;}
	  &save("replace into enso value ('$prod','$appro','$datesimple','$vendu_retoursql','0','1')","af");
      &save("replace into rotation values('$appro','1','$prod','$vendu_retoursql')","af");
    }
}	 
...
if ($action eq "") {
 if (($user eq "sylvain")||($user eq "mireille")||($user eq "edwige")){
