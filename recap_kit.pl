...
if ($action eq "go"){
	...
	while (($code)=$sth->fetchrow_array){
		...
		while (($v_code,$v_rot,$v_vol,$v_date,$v_dest,$v_troltype)=$sthb->fetchrow_array){
			...
			if ($v_rot!=1){
				...
				$ca_rot=&get("select sum(ro_qte*ap_prix/10000) from rotation,appro where ap_code=ro_code and ro_rot='$v_rot' and ap_cd_pr=ro_cd_pr and ap_code='$v_code'");
				print &deci($ca_rot);
				print "</td>";
			}
			...
			if ($v_rot==1){
				...
				$ca_rot=&get("select sum(ro_qte*ap_prix/10000) from rotation,appro where ap_code=ro_code and ro_rot!=1 and ap_cd_pr=ro_cd_pr and ap_code='$v_code'");
				$ca_fly_aff=$ca_fly-$ca_rot;
				print &deci($ca_fly_aff);
				print "</td>";
				...
				$manquante=&get("select sum(montant) from manquante where appro='$v_code'")+0;
				$ecart_caisse+=$manquante;
	...
	print "<tr><th colspan=6>TOTAL</th>";
