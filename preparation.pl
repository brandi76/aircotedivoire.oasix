...
print  "<table border=1 cellspacing=0><tr bgcolor=yellow><th>Jour</th><th>Compagnie</th><th>Vol</th><th>Trolley type</th><th>No lot</th><th>Pre_order</th><th>No appro</th></tr>";
...
while (($liv_vol,$liv_date,$liv_nolot)=$sth->fetchrow_array){
	...
	print "<td>";
	$query="select cde_id from infocmd_web where vol_date='$fl_date' and vol_vol='$fl_vol'";
	$sth2=$dbh->prepare($query);
	$sth2->execute();
	while (($cde_id)=$sth2->fetchrow_array){
		print "<font color=red>$cde_id </font>";
	}	
	print "</td>";
...
print "<a href=avisdep_new.pl?nodepart=$nodepart>Avis de départ et bon d'appro</a><br>";
...
print "<br>";
print "<br>";
print "<a href=avisdep_new.pl?nodepart=$nodepart&printer=11>Avis de départ et bon d'appro sur imprimante de secours</a><br>";
print "<br>";

foreach $trolley (@liste_type){
      if ( $validapp){
		print "<a href=validapp.pl?depart=$nodepart&type=$trolley>Modification du trolley type $trolley</a><br>";
		$query="select tr_cd_pr,pr_desi,tr_qte from trolley,produit where tr_code=$trolley and tr_cd_pr=pr_cd_pr";
		$sth=$dbh->prepare($query);
		$sth->execute();
		while (($pr_cd_pr,$pr_desi,$qte)=$sth->fetchrow_array){
			$qte=int($qte/100);
			$ecart=&get("select ecr_qte from ecartrol where ecr_cdtrol=$trolley and ecr_cd_pr=$pr_cd_pr");
			if ($ecart ne ""){$qte=int($qte/100);}
			%stock=&stock($pr_cd_pr);
			$pr_stre=$stock{"stock"};
			if ($qte >$pr_stre -5){
				print "<span style=color:red>Produit succeptible d'être manquant $pr_cd_pr $pr_desi</span><br>";
			}
			if (($qte==0)&&($pr_stre>12)){
				print "<span style=color:red>Produit en stock mais pas dans la liste $pr_cd_pr $pr_desi</span><br>";
			}
			
		}	
	  }
	  else {
		print "Modification du trolley type $trolley (non actif appro déjà créé)<br>";
	  }
}
