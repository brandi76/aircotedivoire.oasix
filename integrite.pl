...
while (($gsl_nolot,$gsl_apcode)=$sth->fetchrow_array){
	...
	if ($fl_nolot!=$gsl_nolot){	
		...
		print " fl_apcode:$fl_apcode";
		$check=&get("select count(*) from vol where v_code='$gsl_apcode'")+0;
		if ($check==0){ print "inconnu dans vol (libere)";
			if ($option eq "libere"){
				&save("update geslot set gsl_ind=0,gsl_novol='',gsl_apcode='' where gsl_apcode='$gsl_apcode'","aff");
				&save("update etatap set at_etat=-1 where at_code='$gsl_apcode'","aff");
			}
		} 
		print "<br>";
		$err=1;
