...
sub saicaisse{
	...
	$cours_usd=(&get("select valeur from cours_vol where devise='USD'")+0)/$cours_ref;
	if ($cours_usd==0){
	...
	function total_carte()
	{
		...
		document.fiche.nbcb_aff.value=nbcb;
		document.fiche.total_cb_aff.value=total;
	...
	<tr>
	<td align=center ><input type=text  size=4 name=nbcb_aff value=\"$nbcb\" disabled></td><td align=center ><input type=text name=total_cb_aff value=\"$total_cb\" disabled Onchange=\"cal_tot()\">
	<input type=hidden name=nbcb size=4 value=\"$nbcb\"><input type=hidden name=total_cb value=\"$total_cb\">
	</td><td>Tranfert ok <input type=checkbox name=transfert $transfert></td></tr>
...
# 	}
	...
	&save("replace into caissesql values ('$appro','$rotation','$border','$total','$xof','$xaf','$dol','$eur','$total_cb','$nbcb','$stim')");
	...
	# ecart
	if ($justificatif ne ""){
		&save("replace into ecart_caisse values ('$appro','$rotation','$annonce','$justificatif')");
	}
	
	# bancaire
	&save("delete from carte_bancaire where code='$appro' and rot='$rotation'","af");
	for ($i=0;$i<13;$i++){
		if ($mont[$i]!=0){
			&save("insert into carte_bancaire (code,rot,num,montant) values ('$appro','$rotation','$num[$i]','$mont[$i]')","af");
		}
	}
	&page1();
}
