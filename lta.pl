...
{
  print "<h3>$lta</h3>";
  $query="select livh_id,livh_base,livh_four,livh_facture from dfc.livraison_h where livh_lta='$lta'";
  $sth=$dbh->prepare($query);
  $sth->execute();
  print "<table border=1 cellspacing=0><tr><th>Bl</th><th>Base</th><th>Fournisseur</th><th>Facture</th></tr>";
  while (($livh_id,$livh_base,$livh_four,$livh_facture)=$sth->fetchrow_array){
    ($fo_nom,$null)=split(/\*/,&get("select fo2_add from fournis where fo2_cd_fo='$livh_four'"));
    print "<tR><td>$livh_id</td><td>$livh_base</td><td>$livh_four $fo_nom</td><td>$livh_facture</td></tr>";
    $client=$livh_base;
  }
  print "</table>";
