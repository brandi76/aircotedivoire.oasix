...
sub go{
  ...
  while (($code,$rot)=$sth->fetchrow_array){
	...
	$ecart_force=&get("select ecc_montant from ecart_commision where ecc_code='$code' and ecc_rot='$rot'")+0;
	
	if (($ecart_force !=0)&&($ca_recettes+0==0)){$ca_recettes=-1;print "ici";}
	 if (($ca_recettes eq "")or($ca_recettes==0)){next;}
	if ($ca_recettes eq "passe"){$ca_recettes=0;}
	  ...
	  if ($eq_cc ne ""){
		  ...
		  if ($hot_tri eq ""){$eq_cc="$eq_cc";}else{$eq_cc=$hot_tri;}
...
# 	  print &deci($ca_recettes);
	  ...
	  print &deci($com) ;
	  # print "$ecart_force select ecc_montant from ecart_commision where ecc_code='$code' and ecc_rot='$rot'";
	  print "</td></tr>";
	  $recap{$nomtampon}+=$com;
	  $recapnb{$nomtampon}+=1;
  }
