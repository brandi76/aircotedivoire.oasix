...
if ($action eq "Confirmer"){
  &save("insert ignore into releve_bq select id,montant,dev,date,ref,libelle from mouvement_tmp","af");
  $nb=&get("select count(*) from mouvement_tmp")+0;
  print "$nb lignes importées<br>";
  $action="";
}  
if ($action eq "Confirmerdev"){

  &save("delete releve_bq from releve_bq,mouvement_tmp where releve_bq.id=mouvement_tmp.id and releve_bq.montant=mouvement_tmp.montant and releve_bq.date=mouvement_tmp.date","af");
  # $nb=&get("select count(*) from releve_bq,mouvement_tmp where releve_bq.id=mouvement_tmp.id and releve_bq.montant=mouvement_tmp.montant and releve_bq.date=mouvement_tmp.date","af");
...
if ($action eq "import"){
	...
	$ok=0;
	foreach $ligne (@tab){
		($Classification,$Account_number,$Statement,$Amount,$Ccy,$Book_Day,$Type,$Reference,$Description)=split(/\t/,$ligne);
		if ($nb_champ==8){
			($Classification,$Account_number,$Amount,$Ccy,$Value_Day,$Type,$Reference,$Description)=split(/\t/,$ligne);
			$Statement++;
		}
		($null,$null,$Account_number)=split(/ /,$Account_number);
		if ($Account_number eq "006800070006"){next;}
		# if ($Account_number eq "006800070021"){next;}
		if (grep /Classification/,$ligne){
			...
			else
			{
				...
				if (grep /Date.*Date/,$Book_Day){$Book_Day="Date Opération";$Value_Day="Date Valeur";$double=1;}
			...
			if (($nb++<10)&&($ok)){
				...
				{
					$color_stat="";
					if ($Statement eq ""){$color_stat="pink";}
					$color_date="";
					if ($double==1){$Value_Day=substr($Book_Day,10,10);$Book_Day=substr($Book_Day,0,10);}
					$check=substr($Value_Day,0,4)+0;
					# print "*$Value_Day* $check *";
					if (($check <2000)||($check >3000)){$color_date="pink";
						if (grep /\//,$Value_Day){
							($dd,$mm,$yy)=split(/\//,$Value_Day);
							$Value_Day="20".$yy."-".$mm."-".$dd;
							$color_date="";
						}
					}
					print "<tr><td>$Classification</td><td>$Account_number</td><td bgcolor=$color_stat>$Statement</td><td>$Amount</td><td>$Ccy</td><td>$Book_Day</td><td bgcolor=$color_date>$Value_Day</td><td>$Type</td><td>$Reference</td><td>$Description</td></tr>";
				}
			}  
			if (grep /\//,$Value_Day){
				($dd,$mm,$yy)=split(/\//,$Value_Day);
			...
			&save("insert into mouvement_tmp values ('$Statement','$Value_Day','$Ccy','$Amount',\"$Reference\",\"$Description\")","ff"); 
		}
	}
