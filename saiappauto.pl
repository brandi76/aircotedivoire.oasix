...
while (($code)=$sth->fetchrow_array){
	...
	$query="select ap_ordre,ap_cd_pr,pr_desi,ap_qte0/100,ap_prix/100 from appro,produit where ap_cd_pr=pr_cd_pr and ap_code='$code' and ap_cd_pos<5";
	$sth2=$dbh->prepare($query);
	$sth2->execute();
	while (($ap_ordre,$ap_cd_pr,$pr_desi,$ap_qte0,$ap_prix)=$sth2->fetchrow_array){
		...
		$query="select sum(ret_qtepnc-ret_qte),sum(ret_qtepnc-ret_retour)*100 from retoursql where ret_code='$code' and ret_cd_pr=$ap_cd_pr and ret_ordre=$ap_ordre";
		$sth3=$dbh->prepare($query);
		$sth3->execute();
		($ret_ecart,$ret_vendu)=$sth3->fetchrow_array;
		if ($ret_vendu!=0){
			&save("insert ignore into rotation values('$code','1','$ap_cd_pr',0)");
			&save("update rotation set ro_qte=ro_qte+$ret_vendu where ro_cd_pr=$ap_cd_pr and ro_rot=1 and ro_code='$code'");
			# $query="replace into rotation values('$code','1','$ap_cd_pr',$ret_vendu)";
			# &execute();
			&save("insert ignore into enso values('$ap_cd_pr','$code',curdate(),'0','0','1')");
			&save("update enso set es_qte=es_qte+$ret_vendu where es_cd_pr=$ap_cd_pr and es_no_do='$code' and es_dt=curdate()");
...
------------------------------------------------------------
";
	$query="select ap_ordre,ap_cd_pr,pr_desi,ap_qte0/100,ap_prix/100 from appro,produit where ap_cd_pr=pr_cd_pr and ap_code='$code' order by ap_ordre";
	# ici pas fait pas tiroir
	$sth2=$dbh->prepare($query);
	$sth2->execute();
	while (($ap_ordre,$ap_cd_pr,$pr_desi,$ap_qte0,$ap_prix)=$sth2->fetchrow_array){
	    if ($ap_ordre >1000){
			$check=&get("select count(*) from appro where ap_code='$code' and ap_cd_pr='$ap_cd_pr' and ap_ordre <=1000")+0;
			if ($check >0){next;}
		}	
