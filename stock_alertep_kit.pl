...
if ($action eq  "phase1"){
	...
	print "<table border=1 cellspacing=0 cellpadding=0><tr><th colspan=2>Produit</th><th>Pick</th><th>Securite</th><th>Vendu</th><th>Ideal</th></th><th>Stock+Cde</th><th>Proposition</th></tr>";
...
sub algo{
	...
	## modification 23/10/2017
	$vendu_jour=$vendu/21;
	$temps_traitement=10;
	$temps_transport=10;
	$securite=&get("select securite from top where code='$pr_cd_pr'")+0;
	$rythme=14;
	$ideal=($temps_traitement+$temps_transport+$securite+$rythme)*$vendu_jour+$pick;
	$ideal=int($ideal);
	## fin modification 23/10/2017
...
sub ligne_alerte{
	...
	print "<td align=right>$securite</td>";
    print "<td align=right>";
    $color="black";
