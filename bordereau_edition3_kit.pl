...
$premier=$premier.substr($annee,2,2);
$dernier=$dernier.substr($annee,2,2);
if ($premier <10000){$premier="0".$premier;}
if ($dernier <10000){$dernier="0".$dernier;}

print "<center>";
if ($action eq "go") {
	print "<h3>Mois:$date</h3>";
	$query="select distinct ca_border from  vol,caissesql where right(v_date,4)>=$premier and right(v_date,4)<=$dernier and v_code=ca_code";
# 	 print $query;
...
if ($action eq ""){
	...
	print "Annee <input type=text name=annee><br>";
	print "Premier mois <input type=text name=premier><br>";
	print "Dernier mois <input type=text name=dernier><br>";
