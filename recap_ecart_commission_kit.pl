...
if ($action eq "go"){
	...
	foreach (@index){
		...
		if ($nom ne $nomtampon){
			...
			print "<tr><th>Appro</th><th>Vol</th><th>Rotation</th><th>Date</th><th>Equipage</th><th>Dfc retour</th><th>Recettes</th><th>Ecart</th><th>Nb pnc</th><th>Part ecart</th></tr>";
			$nomtampon=$nom;
		}
		...
		print "</td>";
		$retour_dfc=&get("select infr_nom from inforetsql where infr_code='$code'");
		print "<td>$retour_dfc</td>";
		$query="select ca_total,ca_papi from caissesql where ca_code='$code' and ca_rot='$rot'";
		$sth2=$dbh->prepare($query);
		$sth2->execute();
		($ca_recettes,$ca_papi)=$sth2->fetchrow_array;
		...
		$recap{$nomtampon}+=$com;
		$recapnb{$nomtampon}+=1;
		$recap_dfc{$retour_dfc}+=$com;
		$recapnb_dfc{$retour_dfc}+=1/$nb;
		# print "<tr><td>$recap{$nom} *$nomtampon*</td></tr>";
	}
	print "</table><br>";
	&save("create temporary table com_tmp (trig varchar(30),val decimal (8,2) , nb decimal (8,2))");
	...
	$sth2->execute();
	$total=0;
	print "<b>Recapitulatif ACI</b><br> <table border=1 cellspacing=0><tr><th>Pnc</th><th>Valeur</th><th>Moyenne</th></tr>";
	...
	&save("truncate table com_tmp ");
	# $total=0;
	@index= sort (keys(%recap_dfc));
	foreach (@index){
		$nom=$_;
		$val=$recap_dfc{$nom};
		$nb=$recapnb_dfc{$nom};
	...
	$sth2->execute();
	# $total=0;
	print "<b>Recapitulatif DFC</b><br> <table border=1 cellspacing=0><tr><th>Pnc</th><th>Valeur</th><th>Moyenne</th></tr>";
	while (($nom,$val,$nb)=$sth2->fetchrow_array){
		print "<tr><td>$nom </td><td align=right>";
		print $val;
		# $total+=$val;
		print "</td><td align=right>";
		print $nb;
