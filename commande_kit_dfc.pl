...
if ($action eq "send_pdf"){
  ...
  $mail=~s/@/\@/;
  $copie=~s/@/\@/;
  system("/var/www/cgi-bin/dfc.oasix/sendpdf_relcde.pl $mail $fich $com_no $client $copie &");
...
if ($action eq "visu"){
  ...
  if (($pass==0)&&($com2_no_liv !=0)){
    ...
    $query="select livb_code,pr_desi,livb_qte_ent,livb_prix from livraison_b,produit where livb_id='$com2_no_liv' and livb_code=pr_cd_pr";
#     print $query;
    $sth=$dbh->prepare($query);
    $sth->execute();
    while (($com2_cd_pr,$pr_desi,$com2_qte,$com2_prac)=$sth->fetchrow_array){
  ...
  if (-f "/var/www/$client.oasix/doc/$com_no.pdf"){
    print "<a href=http://$client.oasix.fr/doc/$com_no.pdf><img src=/images/pdf.jpg></a>";
  }
  if (-f "/var/www/$client.oasix/doc/${four}_$com_no.pdf"){
    print " <a href=http://$client.oasix.fr/doc/${four}_$com_no.pdf><img src=/images/pdf.jpg></a>";
  }
  if ($etat==0){
    print "<br><form>";
    &form_hidden();
