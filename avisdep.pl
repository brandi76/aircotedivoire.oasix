...
=cut
	...
	# }
	# &save("insert ignore into ecartrol_arch select '$appro',ecr_cd_pr,ecr_qte,pr_stre-pr_stvol from ecartrol,produit where ecr_cdtrol='$fl_troltype' and ecr_cd_pr=pr_cd_pr");
...
sub saiplombs(){
	...
	print "</table><br>Nombre de plombs necessaire:$nbplombs <br>recopie incrementé <input type=checkbox name=recopie checked><input type=hidden name=action value=plombs><input type=hidden name=nodepart value=$nodepart>";
