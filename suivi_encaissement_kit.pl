...
if ($action eq "det_rem"){
	...
	print "<br>";
	print "<table cellspacing=0 border=1>";
 	print "<tr><th>No</th><th>Date</th><th>Montant</th><th>Libelle ";
 	print "</th></tr>";
 	$query="select no,date_remise,montant,ref from bordereau where date_remise >'$date_ref' and date_creation>'$date_ref' and devise='$dev'";
 	$sth=$dbh->prepare($query);
 	$sth->execute();
 	while (($no,$date_remise,$montant,$ref)=$sth->fetchrow_array){
 		print "<tr><td>$no</td><td>$date_remise</td><td align=right>$montant</td><td align=right>$ref</td></tr>";
 		$total_gen+=$montant;
 	}
