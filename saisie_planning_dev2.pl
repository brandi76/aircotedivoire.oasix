...
if ($option eq "recopievol"){
	...
	for ($i=$fl_date+7;$i<$date_finjl;$i=$i+7){
		...
		&save("insert ignore into flyhead value ('$i','$fl_vol','$base_client','$fl_nbrot','$fl_troltype','0','0','0','1','0','$datesql')","check");
		$nb_recopie++;
		...
		while (($flb_date,$flb_vol,$flb_rot,$flb_datetr,$flb_voltr,$flb_depart,$flb_arrivee,$flb_tridep,$flb_triret,$flb_nolot)=$sth->fetchrow_array){
			...
			&save("replace into flybody value ('$i','$flb_vol','$flb_rot','$j','$flb_voltr','0','0','$flb_tridep','$flb_triret','0')","check");
		}
	}
	if ($nb_recopie >0){
...
if ($option eq "updatetroltype"){
	&save("update flyhead set fl_troltype='$fl_troltype' where fl_date='$fl_date' and fl_vol='$fl_vol'","check");
	$action="go";
}
...
if ($action eq "ajoutvol"){
	if ($fl_vol ne ""){
		...
		&save("insert ignore into flyhead value ('$datejl','$fl_vol','$base_client','1','$fl_troltype','0','0','0','1','0','$date')","check");
		&save("insert ignore into flybody value ('$datejl','$fl_vol','10','$datejl','$fl_vol','0','0','$flb_tridep','','0')","check");
	}	
	$action="go";
}	
...
if ($option eq "supvol"){
	...
	if ($check == 0){
		...
		&save("delete from flyhead where fl_date='$fl_date' and fl_vol='$fl_vol' limit 1","check");
		&save("delete from flybody where flb_date='$fl_date' and flb_vol='$fl_vol'","check");
	}
}

if ($option eq "addleg"){
	$flb_datetr=&nb_jour_sql(&datepicker($flb_datetr));
	&save("insert ignore into flybody value ('$fl_date','$param','$flb_rot','$flb_datetr','$flb_voltr','0','0','$flb_tridep','$flb_triret','0')","check");
	$nb_rot=(int(&get("select max(flb_rot) from flybody where flb_date='$fl_date' and flb_vol='$fl_vol'","check")+0)/10);
	&save("update flyhead set fl_nbrot='$nb_rot' where fl_date='$fl_date' and fl_vol='$fl_vol'","check");
...
if ($option eq "modifleg"){
	$flb_datetr=&nb_jour_sql(&datepicker($flb_datetr));
	&save("replace into flybody value ('$fl_date','$param','$flb_rot','$flb_datetr','$flb_voltr','0','0','$flb_tridep','$flb_triret','0')","check");
	$fl_apcode=&get("select fl_apcode from flyhead where fl_vol='$fl_vol' and fl_date='$fl_date'")+0;
	if ($fl_apcode >0){&maj_vol();}
}	
...
if ($option eq "supleg"){
	...
	if ($check==0){		
		$flb_datetr=&nb_jour_sql(&datepicker($flb_datetr));
		&save("delete from flybody where flb_date='$fl_date' and flb_vol='$fl_vol' and flb_rot='$param'","check");
		$nb_rot=(int(&get("select max(flb_rot) from flybody where flb_date='$fl_date' and flb_vol='$fl_vol'","check")+0)/10);
		&save("update flyhead set fl_nbrot='$nb_rot' where fl_date='$fl_date' and fl_vol='$fl_vol'","check");
		$fl_apcode=&get("select fl_apcode from flyhead where fl_vol='$fl_vol' and fl_date='$fl_date'")+0;
		if ($fl_apcode >0){&maj_vol();}
...
print <<EOF;
		            ...
		            <div class="form-group">
						<div class="input-group date form_datetime"  data-date-format="dd MM yyyy - hh:ii " data-link-field="dtp_input1" data-link-format="yyyy-mm-dd-hh-mm" style=width:250px>
							<input class="form-control"  type="text" value="$flb_datetr_sv" readonly>
							<span class="input-group-addon"><span class="glyphicon glyphicon-th"></span></span>
						</div>
						<input type="hidden" id="dtp_input1" value="$flb_datetr_sv" name=flb_datetr /><br/>
					</div>
