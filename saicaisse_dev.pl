...
sub saicaisse{
	...
	$i=0;
	$query="select num,montant from carte_bancaire where code='$appro' and rot='$rotation'";
	$sth2=$dbh->prepare($query);
	$sth2->execute();
	while (($num,$mont)=$sth2->fetchrow_array){
		$num[$i]=$num;
		$mont[$i]=$mont;
		$i++;
	}	
	...
	function total_carte()
	{
		total=0;
		nbcb=0;
		for (i=0;i<12;i++){
			mont=+document.fiche['mont_'+i].value;
			total+=mont;
			if (mont>0){nbcb++;}
		}
		document.fiche.total_cb.value=total;
		document.fiche.nbcb.value=nbcb;
		cal_tot();
	}
	...
	<table>
	<tr><th>No carte</th><th>Montant</th></tr>";
	for ($i=0;$i<12;$i++){
		print "<tr><td align=center ><input type=text name=num_$i size=16 value=\"$num[$i]\"></td> <td align=center ><input type=text name=mont_$i size=16 value=\"$mont[$i]\" Onchange=\"total_carte();\" name=mont_$i></td></tr>\n";
	}
	print "</table>
	<table>
	<tr><th>Carte bancaire</th><th>Total</th></tr>
	<tr><td align=center ><input type=text name=nbcb size=4 value=\"$nbcb\" disabled></td><td align=center ><input type=text name=total_cb value=\"$total_cb\" disabled Onchange=\"cal_tot()\"></td><td>Tranfert ok <input type=checkbox name=transfert $transfert></td></tr>
...
sub enregistre{
	# &save("insert ignore into traceur value (now(),'saicaisse $appro',\"$ENV{\"REMOTE_USER\"}\",\"$ENV{\"REMOTE_ADDR\"}\")","check");
	...
	for ($i=0;$i<13;$i++){
		$num[$i]=$html->param("num_$i");
		$mont[$i]=$html->param("mont_$i")+0;
	}
...
# 	 if ($xof!=":::::") {
		# $sth=$dbh->prepare("insert ignore into bordereau values ('$border','XOF',curdate(),'','','','')","check");
		$sth->execute();
# 	}
# 	if ($xaf!="::::") {
		# $sth=$dbh->prepare("insert ignore into bordereau values ('$border','XAF',curdate(),'','','','')","check");
		$sth->execute();
# 	}
# 	if ($dol!="::::::") {
		# $sth=$dbh->prepare("insert ignore into bordereau values ('$border','USD',curdate(),'','','','')","check");
		$sth->execute();
# 	}
# 	if ($eur!="::::::") {
		# $sth=$dbh->prepare("insert ignore into bordereau values ('$border','EUR',curdate(),'','','','')","check");
		$sth->execute();
# 	}
	...
	# &save("replace into caissesql values ('$appro','$rotation','$border','$total','$xof','$xaf','$dol','$eur','$total_cb','$nbcb','$stim')","check");
	if ($html->param("transfert") eq "on"){
	  # &save("replace into transfertcb value ('$appro','$rotation','$total_cb')","check");
	  }
	else
	  {
	  # &save("delete from transfertcb  where tra_code='$appro' and tra_rot='$rotation'","check");
	}
	
	# ecart
	if ($justificatif ne ""){
		# &save("replace into ecart_caisse values ('$appro','$rotation','$annonce','$justificatif')","check");
	}
	
	# bancaire
	&save("delete from carte_bancaire where code='$appro' and rot='$rot'","aff");
	for ($i=0;$i<13;$i++){
		if ($mont[$i]!=0){
			&save("insert into carte_bancaire (code,rot,num,montant) values ('$appro','$rotation','$num[$i]','$mont[$i]')","aff");
		}
	}
	&page1();
}
...
sub bordereau{
	...
	while (($appro,$rotation,$total,$ca_xof,$ca_xaf,$ca_dol,$ca_eur,$total_cb,$nbcb,$stim)=$sth->fetchrow_array){
		...
		$sth2=$dbh->prepare($query);
		$sth2->execute();
		while (($num[$i],$mont[$i])=$sth2->fetchrow_array){
			$i++;
		}	
		$xof_1+=0;
		$xof_2+=0;
