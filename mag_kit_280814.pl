...
if (($action eq "ins")&&($code ne "")){
    ...
    print "<div class=titre><input type=text name=code value=\"$code\"> $pr_desi</div>";
   
   &form_hidden();
    print "Prix <input type=text name=prix value=\"$prix\" style=position:absolute;left:300px><br />";
    $conv=$prix*655.957;
    print "Prix XOF ($conv) <input type=text name=prix_xof value=\"$prix_xof\" style=position:absolute;left:300px><br />";
    if ($code==0){
      print "Designation<br><input type=text name=desi value=\"$desi\"  size=80><br />";
    }
    print "Marque<br><input type=text name=marque value=\"$marque\" size=80<br />";
    print "Texte <input type=checkbox name=texte $textecheck style=position:absolute;left:300px><br />";
    print "Visuel <input type=checkbox name=visuel $visuelcheck style=position:absolute;left:300px><span style=position:absolute;left:370px>Prix</span><input type=text name=visuelprix value='$visuelprix' style=position:absolute;left:400px size=4><br />";
    print "Pub <input type=checkbox name=pub $pubcheck style=position:absolute;left:300px><span style=position:absolute;left:370px>Prix</span><input type=text name=pubprix value='$pubprix' style=position:absolute;left:400px size=4><br />";
    print "Designation Pub<br><input type=text name=desi_pub value=\"$desi_pub\" size=80<br />";
    print "Nouveau <input type=checkbox name=new $newcheck style=position:absolute;left:300px><br />";
    print "Info<br><input type=text name=info value=\"$info\"  size=80><br />";
    print "Contact<br><input type=text name=contact value=\"$contact\" size=80><br />";
    print "<input type=hidden name=mag value=$mag>";
...
if (($action eq "modif_save")||($action eq "modif_save_verif1")||($action eq "modif_save_verif2")||($action eq "modif_save_verif3")){
    if ($cases eq ""){
      ...
      &save("insert into mag values ('$mag','$page','$cases','$code','$prix','$prix_xof','$texte','$visuel','$pub','$new','$info','$contact','$desi','$visuelprix','$pubprix','$desi_pub','$marque')","aff");
    }
    else {
    ...
    $focus=$code;
    if ($action eq "modif_save") {$action="go";}
...
if ($action eq "facture_pub"){
  ...
  if ($facture eq ""){print "merci de mettre un numero de facture<br>";$action="pub";}
	...
	if ($html->param("$ref") eq "on"){
	  &create_pdf();
	  print "$nom <a href=http://$base_rep.fr/doc/pub_$facture.pdf><img src=/images/pdf.jpg></a><br>";
	  $facture++;
	}
     }
     else
     {
	$marqueindex=0;
	$query = "select distinct marque from mag,produit where pr_cd_pr=code and (pub=1 or visuel=1) and pr_four='$four' and mag='$mag' order by marque";
	$sth2=$dbh->prepare($query);
	$sth2->execute();
	while (($marque)=$sth2->fetchrow_array){
	  ...
	  if ($html->param("$ref") eq "on"){
	    &create_pdf();
	    print "$nom $marque <a href=http://$base_rep.fr/doc/pub_$facture.pdf><img src=/images/pdf.jpg></a><br>";
	    $facture++;
	  }
	  $marqueindex++;
	}
...
sub create_pdf{  
  ...
  $index=0;
  &facture_suite();
  $query = "select code,pr_desi,visuel,pub,visuelprix,pubprix from mag,produit where pr_cd_pr=code and (pub=1 or visuel=1) and mag='$mag' and pr_four='$four' order by pr_desi";
  $sous_tot=0;
  if ($four==1260){$sous_tot=1;} #distrimark
  if ($four==1290){$sous_tot=1;} #iom
  if ($sous_tot==1){
    $query = "select code,pr_desi,visuel,pub,visuelprix,pubprix from mag,produit where pr_cd_pr=code and (pub=1 or visuel=1) and mag='$mag' and pr_four='$four' and marque='$marque' order by pr_desi";
  }
  my($sth)=$dbh->prepare($query);
  $sth->execute();
  $first=0;
  while (($code,$pr_desi,$visuel,$pub,$prix,$pubprix)=$sth->fetchrow_array){
...
sub facture_suite{
  ...
  $tete_text->translate( 20/mm, 225/mm );
  $tete_text->text("Magazine:$cl_magazine N°:$mag");
  $tete_text->translate( 20/mm, 215/mm );
  $tete_text->text("Compagnie $cl_nom");
  if ($sous_tot==1){
    $tete_text->translate( 80/mm, 215/mm );
