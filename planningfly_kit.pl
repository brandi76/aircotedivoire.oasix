...
if ($action eq "validmodif"){
	...
	# rotation supplementaire
		...
		if ($fl_vol ne $volhead){
		    $check=&get("select count(*) from flyhead where fl_vol='$volhead' and fl_vol='$vol' and fl_date=$date")+0;
			if ($check==0){
	...
	# 			$query="update flybody set flb_voltr='$volhead' where flb_vol='$volhead' and flb_date=$date and flb_rot=11";  
				...
				print "<font color=red> Impossible il existe deja un vol avec ces  informations</font></br>";
			}
		}
