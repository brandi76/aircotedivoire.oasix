...
if (($action eq "creation") or ($action eq "clone")){
	...
	{	
	  foreach $client (@bases_client) {
	    &save ("INSERT ignore into $client.produit  select '$nouveau','nouveau','0','0','0','pr_douane','pr_ventil','0','pr_type','pr_prx_vte','0','pr_sup','pr_emb','0','pr_deg','pr_pdn','0','pr_acquit','pr_orig','pr_pdb','pr_qte_comp','pr_cond','pr_devac','pr_four','','' from dfc.produit where pr_cd_pr='$pr_cd_pr'","af");
...
# 		&save("update $client.produit set pr_desi='$pr_desi',pr_sup='$pr_sup',pr_codebarre='$pr_codebarre',pr_type='$pr_type' where pr_cd_pr='$pr_cd_pr'");
 		&save("update $client.produit set pr_desi='$pr_desi',pr_pdn='$pr_pdn' where pr_cd_pr='$pr_cd_pr'");
	...
	if ($achat eq "on"){
		...
		$pr_prac*=100;
		foreach $client (@bases_client) {
		  &save("update $client.produit set pr_prac='$pr_prac',pr_prx_rev='$pr_prx_rev',pr_four='$pr_four',pr_refour='$pr_refour',pr_prx_vte='$pr_prx_vte' where pr_cd_pr='$pr_cd_pr'");
...
sub table_logistique {
	...
	if ($option==0){print "<input type=text $readonly name=marque value='$marque' style=\"background-color:$color;;text-align:left;\">";}
	  ...
	  print "<select name=marque style=;text-align:left;>";
   	  $query="select marque from marque_desi order by marque";
