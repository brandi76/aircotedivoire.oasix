...
if ($action eq "add"){
	...
	&save("insert ignore into panier values('$cle','$code','0','$prix')","af");
	&save("update panier set qte=qte+1 where cle='$cle' and code='$code'","af");
	$action="";
	$modal=1;
	$code_modal=$code;
}
...
if (($action eq "aff_panier")||($action eq "modal")){
	print <<EOF;
	<div class="modal-header">
	<button type="button" class="close" data-dismiss="modal">&times;</button>
	</div>
...
EOF
	if ($action eq "modal"){
		print "Produit mis dans votre panier.<bR>";
		$query="select code,pr_desi,prix,prix_xof from mag,produit where mag='$mag' and  produit.pr_cd_pr=code and code='$code'";
		$sth=$dbh->prepare($query);
		$sth->execute();
		($code,$desi,$prix,$prix_xof)=$sth->fetchrow_array;
		...
		if ($texte_f eq ""){$texte_f="Lorem ipsum dolor sit amet, consectetur adipiscing elit.";}
		if ($image_l eq ""){$image_l="320x150.png";}
		print "<img src=/images/$image_s alt=\"\" style=width:150px><h5>$desi $prix €</h5>";
	}
...
EOF
	...
	print "<br> Vous pouvez regler votre commande par carte bancaire en cliquant ici, dans le cas contraire le réglement se fera à bord</b>";
	print "<br><b>* Merci d'avoir fait vos achats en ligne avec nous !</b><br>";
	print "<input type=submit value=\"Retour Site $compagnie\">";
	print "</form>";
	print "<form>";
	print "<input type=hidden name=action value=cb><Button type=submit class=\"btn btn-primary\"><img src=/images/cb.png width=50px></button>";
	print "</form>";
	print "</div>";
	print "</div>";
print <<EOF;
...
$query="select sum(qte),sum(qte*prix) from panier where cle='$cle'";
$sth=$dbh->prepare($query);
$sth->execute();
...
if ($action eq "cb"){&carte_bancaire();}
...
sub carte_bancaire
{
	...
	print "<p style=font-size:1.3em;color:white>Reglement par carte bancaire</p>";
	print "<p style=color:white>";
	$montant=&get("select sum(qte*prix) from panier_web where cde_id='$cde_id'","af")+0;
	# print "$cde_id $montant";
	if (($montant <10)||($montant>=1000)){
		print "<div class=\"alert alert-danger\">Désolé une erreur est survenue dans le montant, vote demande ne pourra pas être prise en compte, le paiement devra se faire à bord</div>";	
	}	
	else{
	print "Montant:$montant Eur<br>";
	print "Cliquez sur le logo CB , vous allez être redirigé sur le site sécurisé de notre partenaire bancaire, un mail de confirmation vous sera envoyé<br>";
	$montant*=100;
	...
	$vads_url_sucess="http://dutyfreeambassade.com/cgi-bin/cb_mode_test.pl";
	$vads_version="V2";
	$vads_order_id="$cde_id";
	$certificat="3458311677213718";
	$cle=$vads_action_mode."+".$vads_amount."+".$vads_ctx_mode."+".$vads_currency."+".$vads_order_id."+".$vads_page_action."+".$vads_payment_config."+".$vads_site_id."+".$vads_trans_date;
	$cle.="+".$vads_trans_id."+".$vads_url_success."+".$vads_version."+".$certificat;
	...
	print "<input type=hidden name=vads_order_id value=$vads_order_id>";
	...
	print "<input type=hidden name=vads_url_success value=$vads_url_success >";
	print "<input type=hidden name=vads_version value=$vads_version>";
	print "<input type=hidden name=signature value=$signature>";
	print "<Button type=submit class=\"btn btn-primary\"><img src=/images/cb.png></button>";
	print "</form>";
	}
	print "</div></div>";
}
...
if ($modal){
	print <<EOF;	
	<script>
	\$("#infos").modal({ remote: "${script_name}?action=modal&code=$code_modal" }, "show");			
	</script>
EOF
}
if ($modal_panier){
	print <<EOF;	
	<script>
	\$("#infos").modal({ remote: "${script_name}?action=aff_panier" }, "show");			
	</script>
EOF
}
...
if ($envoyer){
	print <<EOF;	
	<script>
	\$("#infos").modal({ remote: "${script_name}?action=aff_confirm&mail=$mail" }, "show");			
	</script>
EOF
}
print "</body></html>";
