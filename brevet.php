...
if ($raz == "on"){
	spip_query("update pilote2 set ko=0 where nom='$nom'");
}
if ($raztot == "on"){
	spip_query("delete from pilote2 where nom='$nom'");
}
if ($pasglop == "on"){
	spip_query("update pilote2 set pasglop=1 where nom='$nom' and no='$no'");
}
...
if ($nom == ""){
	$affichage_php= "<h3>".htmlentities("Révise ton brevet avec Normandie Vol Libre")."</h3><br>";
	...
	$affichage_php.="<br><select name=type_query><option value='%'>Tous les sujets</option><option value='A'>";
	$affichage_php.=htmlentities("Météo");
	$affichage_php.="</option><option value='E'>";
	$affichage_php.=htmlentities("Mécavol Général");
	$affichage_php.="</option><option value='G'>";
	$affichage_php.=htmlentities("Mécavol Parapente");
	$affichage_php.="</option><option value='H'>";
	$affichage_php.=htmlentities("Mécavol Delta");
	$affichage_php.="</option>";
	$affichage_php.="<option value='L'>";
	$affichage_php.=htmlentities("Matériel Général");
	$affichage_php.="</option><option value='N'>";
	$affichage_php.=htmlentities("Matériel Parapente");
	$affichage_php.="</option><option value='R'>";
	$affichage_php.=htmlentities("Matériel Delta");
	$affichage_php.="</option><option value='S'>";
	$affichage_php.=htmlentities("Réglementation");
	$affichage_php.="</option>";
	$affichage_php.="<option value='U'>";
	$affichage_php.=htmlentities("Pilotage Général");
	$affichage_php.="</option><option value='W'>";
	$affichage_php.=htmlentities("Pilotage Parapente");
	$affichage_php.="</option><option value='X'>";
	$affichage_php.=htmlentities("Pilotage Delta");
...
$affichage_php.="</br><br>Uniquement les questions sur lesquelles il reste une erreur <input type=checkbox name=choixcheck><br>";
	...
	$affichage_php.= "<br><br><select name=niveau><option value='%' >Tous les niveaux (680 questions) </option>";
	$affichage_php.= "<option value='V'> Niveau Vert , Brevet initial (159 questions)</option>";
	$affichage_php.= "<option value='B'> Niveau Bleu , Pilote autonome (347 questions)</option>";
	$affichage_php.= "<option value='M'> Niveau Marron , Pilote confirm".htmlentities("é")."(172 questions)</option>";
	$affichage_php.= "</select>";
	$affichage_php.= "<br><br><input type=submit>";
	$affichage_php.= "</form>";
	$affichage_php.= "<br><br><i>".htmlentities("Note: Comme à chaque nouvelle version le fichier de la ffvl doit être revu et corrigé, merci à laurent pour ce travail")."</i>";
	}
else
{
	$affichage_php= "<br><br>";
	...
	if ($actino == "go"){
		...
		if ($note==6){ 
			...
			$affichage_php.= "<br>";
			$affichage_php.= "<br>".htmlentities("Si tu n'es pas d'accord avec la réponse coche ici")."<input type=checkbox name=pasglop><br>";
		...
		$affichage_php.= "<br><br>Nombre de point obtenu:<b>$note</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font size=+2>$valeur/20";
		if ($valeur >=15) {$affichage_php.= htmlentities(" Reçu")."</font>";}
		else {$affichage_php.= htmlentities(" Recalé")."</font>";}
		$affichage_php.= "<input type=hidden name=res value='$res'><input type=hidden name=total value='$total'>";
