...
if ($action eq "ajoutvol"){
	if ($fl_vol ne ""){
		...
		&save("insert ignore into flybody value ('$datejl','$fl_vol','11','$datejl','$fl_vol','0','0','$flb_tridep','','0')","chec");
	}	
	$action="go";
}	
...
if ($action eq "go"){
	...
	while (($fl_date,$fl_vol,$fl_cd_cl,$fl_nbrot,$fl_troltype,$fl_nbtrol,$fl_troltypebis,$fl_nolot,$fl_part,$fl_apcode)=$sth->fetchrow_array){
		$show="";
	    if ((($option eq "ajouttron")||($option eq "modiftron"))&&($fl_vol ne $param2)) {$show="style=display:none";}
		if (($option eq "ajoutleg")&&($fl_vol ne $param)){$show="style=display:none";}
		print "<tr $show><td>*";
