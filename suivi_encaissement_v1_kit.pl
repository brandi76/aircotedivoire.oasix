	...
	$color="white";
 	...
 	while (($no,$date_remise,$montant,$ref,$montant_dev)=$sth->fetchrow_array){
		...
		($ref_bon,$null)=split(/\//,$ref);
		  ...
		  $query="select distinct(montant) from releve_bq where ref= '$ref_bon'";
		  # print "$query";
		  $sth2=$dbh->prepare($query);
		  $sth2->execute();
		  $montant_bq=0;
...
# 	}
	...
	print "<tr><td colspan=2><b>Total</b></td><td align=right><b>$total_gen</b></td><td align=right><b>$total_cash</b></td><td>&nbsp;</td><td align=right><b>$total_ecart</b></td></tr>";
