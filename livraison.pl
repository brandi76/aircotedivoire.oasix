...
if ($action eq "print"){
	...
	$today--;
	$nb=&get("select count(*) from vol,livraison where liv_no=$noliv and liv_code=v_code and v_rot=1 and v_dest like 'ABJ%' and v_zatt!='AN'");
	if ($nb>0){	
		&bord_liv("ABJ");
		print "Fin de page";
		# exit;
	...
	if ($paris){
		...
		$nb=&get("select count(*) from geslot where gsl_dtret<=$today+1 and gsl_ind=3 and gsl_triret=\"ABJ\"");
                if ($nb>0){
			print "<h4>.</h4>";  # saut de page
			&tableretour($today+1,"ABJ");
			print "Fin de page";
			print "</center>";
		}
	}
	...
	if ($paris){
		...
		$query="select v_date_jl from vol,livraison where liv_no=$noliv and liv_code=v_code and v_rot=1 and v_dest like 'ABJ%' and v_zatt!='AN' group by v_date_jl";
		$sth3=$dbh->prepare($query);
		$sth3->execute();
		while (($jour)=$sth3->fetchrow_array){
			$nb=&get("select count(*) from geslot,flybody where gsl_dtvol=$jour and flb_date=gsl_dtvol and flb_vol=gsl_novol and flb_rot=11 and (gsl_trajet like \"/ABJ%\" or gsl_trajet like \"ABJ%\")");
			if ($nb>0){
				print "<h4>.</h4><center>Document Piste ABJ exemplaire pour <b>ORLY</b>";  # saut de page
				&tablepiste($jour,"ABJ");
				print "Fin de page";
				print "<h4>.</h4><center>Document Piste ABJ exemplaire pour <b>ABJ</b>";  # saut de page
				&tablepiste($jour,"ABJ");
				print "Fin de page";
			}
		}
...
sub premiere{
	...
	for($i=0;$i<=21;$i++) {
		$jour=$today+$i;
		$nbvol=&get("select count(*) from vol,etatap where v_date_jl=$jour and at_etat=2 and at_code=v_code and v_rot=1 and v_dest like 'ABJ%' and v_zatt!='AN'","af");
		if ($nbvol >0){		
...
sub creation{
	...
	for ($i=0;$i<=21;$i++){
		...
		if ($check eq "on"){
			$query="select v_code from vol,etatap where v_date_jl=$jour and at_etat=2 and at_code=v_code and v_dest like 'ABJ%' and v_rot=1 and v_zatt!='AN'";
			$sth=$dbh->prepare($query);
			$sth->execute();
