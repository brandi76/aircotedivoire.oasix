...
if ($action eq "go") {
	...
	$sth->execute();
	print "<table><tr><th>No</th><th>Devise</th><th>Date de creation</th><th>Date de remise</th><th>Reference</th><th>Montant saisie</th><th>Montant remis</th></tr>";
	while (($no,$devise,$date_creation,$date_remise,$ref,$montant)=$sth->fetchrow_array){
		...
		print "<tr><td>$no</td><td>$devise</td><td>$date_creation</td><td>$date_remise</td><td>$ref</td><td>$saisie</td><td>$montant</td></tr>";
		$total_saisie+=$saisie;
		$total_montant+=$montant;	
	}
	print "<tr><th colspan=5>Total</th><th>$total_saisie</th><th>$total_montant</th></table>";
